...
def ST_CallBack():
  ...
  if StaStatus == 0:
    ...
    StaStatus = 0 
    ST_Button.configure(text = "Start")
...
def main():
  ...
  while 1:
    if n == 10000:
      ...
      try:
        s.send('TEM 0')
	TempSens1 = s.recv(BUFFER_SIZE)[0:2]
        Temp1.set('T1: %sC'%TempSens1)
        s.send('TEM 1')
	TempSens2 = s.recv(BUFFER_SIZE)[0:2]
        Temp2.set('T2: %sC'%TempSens2)
        s.send('TEM 2')
	TempSens3 = s.recv(BUFFER_SIZE)[0:2]
        Temp3.set('T3: %sC'%TempSens3)
        s.send('TEM 3')
	TempSens4 = s.recv(BUFFER_SIZE)[0:2]
        Temp4.set('T4: %sC'%TempSens4)
        s.send('TEM 7')
	TempSens5 = s.recv(BUFFER_SIZE)[0:2]
        Temp5.set('T5: %sC'%TempSens5)
        if p == 10000:
          p = 0
          TempFile = open("TempLog.txt", "a")
          TempFile.write('%s -> Sens1 %sC, Sens2 %sC, Sens3 %sC, Sens4 %sC, Sens5 %sC\r\n'%(datetime.datetime.now(), TempSens1, TempSens2, TempSens3, TempSens4, TempSens5))
          TempFile.close
          print('%s -> Sens1 %sC, Sens2 %sC, Sens3 %sC, Sens4 %sC, Sens5 %sC'%(datetime.datetime.now(), TempSens1, TempSens2, TempSens3, TempSens4, TempSens5))
      except:
        Temp1.set('T1: -1C')
        Temp2.set('T2: -1C')
        Temp3.set('T3: -1C')
